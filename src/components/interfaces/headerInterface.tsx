...
export default function HeaderInterface() {
  ...
  const { isDarkMode, language, bgColor, textColor } = getPortfolioState;
  ...
  return (
    <div className={`max-h-[10vh] flex justify-between items-center pr-4 md:pl-4 fixed w-full ${textColor} z-20 ${bgColor} md:backdrop-blur-md md:bg-opacity-60`}>
      ...
      <HeaderMenuInterface />
      <div className="flex items-center justify-center gap-2 w-36 h-[10vh]">
        <button
          onClick={() => setPortfolioState(prevState => ({ ...prevState, language: prevState.language === 'es' ? 'en' : 'es' }))}
        ...
        >
          <span className={`absolute w-5 h-5 rounded-full transition-all duration-300 shadow-sm ${language === 'en' ? "translate-x-7 bg-cvButtonSecondary/60" : "translate-x-0 bg-cvButtonPrimary/60"}`} />
          <span className={`z-10 w-5 h-5 flex items-center justify-center text-[10px] font-bold transition-all duration-200 ${language === 'es' ? "text-white" : "text-white/30"}`}>ES</span>
          <span className={`z-10 w-5 h-5 flex items-center justify-center text-[10px] font-bold transition-all duration-200 ${language === 'en' ? "text-cvButtonPrimary" : "text-cvButtonPrimary/40"}`}>EN</span>
        </button>
        <button
