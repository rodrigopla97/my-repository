  ...
  return (
    ...
    >
      ...
      >
        {/* Mobile header */}
        <div
          className={`flex md:hidden items-center gap-1 px-4 py-5 ${isDarkMode ? "bg-white/10" : "bg-black/10"}`}
        >
          <button
            type="button"
            onClick={modal.close}
            className={`flex-shrink-0 flex items-center transition-opacity hover:opacity-60 ${textColor}`}
          >
        ...
        {/* Desktop header */}
        <div
          className={`hidden md:relative md:flex md:items-center md:justify-center px-4 py-5 ${isDarkMode ? "bg-white/10" : "bg-black/10"}`}
        >
          <h2 className={`text-sm font-semibold uppercase tracking-widest ${textColor}`}>
            {title}
          </h2>
        ...
        {footerActions && (
          <div
            className={`flex items-center justify-end gap-3 px-5 py-4 border-t ${isDarkMode ? "border-white/10" : "border-black/10"}`}
          >
            <button
              type="button"
              onClick={modal.close}
