...
function NotifIcon({ type }: { type: NotificationType }) {
  ...
  if (type === "success") return <span className={cls}><CheckIcon /></span>;
  if (type === "info") return <span className={cls}><InfoIcon /></span>;
  return <span className={cls}><ErrorIcon /></span>;
}
  ...
  return (
    <div className="fixed top-[3vh] right-8 z-50 animate-fadeIn">
      <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border-l-4 bg-neutral-900 shadow-xl min-w-64 max-w-xs ${borderStyles[type]}`}>
        ...
        <button type="button" onClick={closeNotification} className="text-neutral-400 hover:text-neutral-200 transition-colors flex-shrink-0">
          <CloseIcon />
        </button>
      </div>
    </div>
