...
export default function HeaderInterface() {
  ...
  return (
    <div
      className={`max-h-[10vh] flex justify-between items-center pr-6 md:pl-4 fixed w-full ${textColor} z-30 ${bgColor} md:backdrop-blur-md md:bg-opacity-60`}
    >
      ...
      <div className="flex items-center justify-center gap-2 h-[10vh]">
        <button
          type="button"
          onClick={() =>
            setPortfolioState((state) => ({ ...state, isDarkMode: !state.isDarkMode }))
          }
        ...
        >
          <span
            className={`absolute w-5 h-5 rounded-full transition-all duration-300 shadow-sm ${isDarkMode ? "translate-x-7 bg-cvButtonSecondary/60" : "translate-x-0 bg-cvButtonPrimary/60"}`}
          />
          <span className="z-10 w-5 h-5 flex items-center justify-center">
            <i
              className={`material-symbols-outlined text-base transition-all duration-200 ${!isDarkMode ? "text-white" : "text-white/30"}`}
            >
              light_mode
            </i>
          </span>
          <span className="z-10 w-5 h-5 flex items-center justify-center">
            <i
              className={`material-symbols-outlined text-base transition-all duration-200 ml-1 ${isDarkMode ? "text-cvButtonPrimary" : "text-cvButtonPrimary/40"}`}
            >
              dark_mode
            </i>
          </span>
